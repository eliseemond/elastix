...
TransformBase<TElastix>::WriteToFile(xl::xoutsimple & transformationParameterInfo, const ParametersType & param) const
{
  ...
  const auto itkTransformOutputFileNameExtension = getFront(configuration.GetValuesOfParameter("ITKTransformOutputFileNameExtension"));
  ...
  if (!itkTransformOutputFileNameExtension.empty())
  {
    ...
    if (firstSingleTransform != nullptr)
    {
      ...
      const auto transformFileName =
        ...
        itkTransformOutputFileNameExtension.front();
      ...
      writeCompositeTransform->front() && !itkTransformOutputFileNameExtension.empty())
  {
    ...
    else
    {
      TransformIO::Write(*compositeTransform,
                         std::string(m_TransformParametersFileName, 0, m_TransformParametersFileName.rfind('.')) +
                           "Composite" + itkTransformOutputFileNameExtension.front());
    }
  }
